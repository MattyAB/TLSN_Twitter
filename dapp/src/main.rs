...
struct TwitterApp {
    ...
    number_of_followers_memory: Mutex<HashMap<Address, u64>>,
}

impl TwitterApp {
    fn new() -> Self {
        Self {
            number_of_followers_memory: Mutex::new(HashMap::new()),
        }
    }
}
...
impl Application for TwitterApp {
    async fn advance(
        ...
        match input {
            Input::ProofOfOwnership { username, proof } => {
                // Verify the proof of ownership
                verify_proof_of_ownership(username.clone(), proof)?;

                // Associate the sender's address with the username
                let caller_address = metadata.sender;
                println!(
                    "Address {:?} proved ownership of the username {}",
                    caller_address, username
                );

                

                // Optionally, store this association in a map
                // For simplicity, we're not storing it here
            }
            Input::ProofOfTheNumberOfFollowers { username, proof } => {
                // Verify the proof of the number of followers
                let follower_count = verify_twitter_followers(username.clone(), proof)?;

                // Update the number of followers in the in-memory map
                let caller_address = metadata.sender;
                {
                    let mut memory = self.number_of_followers_memory.lock().unwrap();
                    memory.insert(caller_address, follower_count.follower_count);
                }
    ...
    async fn inspect(
        ...
        let memory = self.number_of_followers_memory.lock().unwrap();

        // Assume the payload contains the address you're interested in
        // Deserialize the address from the payload
        let address: Address = serde_json::from_slice(payload)?;

        // Get the follower count for the given address
        if let Some(&follower_count) = memory.get(&address) {
            println!(
                "Address {:?} has {} followers.",
                address, follower_count
            );
        } else {
            println!(
                "No follower count found for address {:?}.",
                address
            );
        }
...
fn verify_twitter_followers(
    target_username: String,
    proof: TlsProof,
) -> Result<FollowerCount, Box<dyn std::error::Error>> {
    let TlsProof { session, substrings } = proof;
