...
fn verify_twitter_followers(target_username: String) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("{}", user_json_obj.get("data").and_then(|x| x.get("user")).unwrap());

    // if let Some(screen_name) =  {
    //     if screen_name.to_lowercase() != target_username.to_lowercase() {
    ...
    println!("{}", String::from_utf8(auth_proof_recv.data().to_vec()).unwrap());
    println!("{}", String::from_utf8(user_proof_recv.data().to_vec()).unwrap());
